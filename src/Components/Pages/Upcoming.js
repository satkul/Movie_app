...
const Upcoming = () => {
    const fetchData = async (page) => {
        const { data } = await axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=${KEY}&language=en-US&page=${page}`)
    ...
    const {
        ...
        status,
        error
    } = useInfiniteQuery('upcoming', ({ pageParam = 1 }) => fetchData(pageParam), {
